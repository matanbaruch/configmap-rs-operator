...
import (
	...
	"github.com/matanbaruch/configmap-rs-operator/test/utils"
)

// namespace where the project is deployed in
const namespace = "configmap-rs-operator-system"

// serviceAccountName created for the project
const serviceAccountName = "configmap-rs-operator-controller-manager"

// metricsServiceName is the name of the metrics service of the project
const metricsServiceName = "configmap-rs-operator-controller-manager-metrics-service"

// metricsRoleBindingName is the name of the RBAC that will be created to allow get the metrics data
const metricsRoleBindingName = "configmap-rs-operator-metrics-binding"
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				"--clusterrole=configmap-rs-operator-metrics-reader",
		...
		It("should add owner reference to ConfigMap when ReplicaSet mounts it", func() {
			testNamespace := "test-configmap-rs"
...
`, configMapName, testNamespace)
			...
			By("creating a ReplicaSet that mounts the ConfigMap")
			replicaSetYaml := fmt.Sprintf(`
apiVersion: apps/v1
kind: ReplicaSet
metadata:
  name: %s
...
`, replicaSetName, testNamespace, configMapName)
			...
			By("verifying that the ConfigMap has owner reference to the ReplicaSet")
			verifyOwnerReference := func(g Gomega) {
				...
				err = json.Unmarshal([]byte(output), &configMap)
				g.Expect(err).NotTo(HaveOccurred())
				...
				ownerRef := ownerReferences[0].(map[string]interface{})
				g.Expect(ownerRef["kind"]).To(Equal("ReplicaSet"))
				g.Expect(ownerRef["name"]).To(Equal(replicaSetName))
			}
			Eventually(verifyOwnerReference, 2*time.Minute).Should(Succeed())
